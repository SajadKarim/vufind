...
function checkItemStatuses(_container) {
  ...
  var data = $.map(container.find('.ajaxItem'), function ajaxItemMap(record) {
	if ($(record).find('.hiddenCatId').val() != 'GEI') {//scheel: only ask gbv for GEI availabilities
		// No data is available -- hide the entire status area:
//		console.log($(record).find('.callnumAndLocation'));
        $(record).find('.callnumAndLocation').addClass('hidden');
        $(record).find('.status').addClass('hidden');
		//scheel: fallback to use indexed holdings
		$(record).find('.location2').removeClass('hidden');
    }
    if ($(record).find('.hiddenPPN').length === 0) {//scheel: was '.hiddenId'
      return null;
    }
    var datum = $(record).find('.hiddenPPN').val();//scheel: was '.hiddenId'
    ...
    $.each(response.data, function checkItemDoneEach(i, result) {
      ...
      if (typeof(result.full_status) != 'undefined'
        ...
        item.find('.locationDetails').empty().append(locationListHTML);
		  ...
		  //scheel: quickfix
		  if(result.location == 'Braunschweig Georg-Eckert-Inst'){
			  result.location = 'Braunschweig Georg-Eckert-Institut';
		  }
