...
function buildFacetTree(treeNode, facetData, inSidebar, facets_before_more) {
  ...
  treeNode.on('loaded.jstree open_node.jstree', function treeNodeOpen(/*e, data*/) {
    treeNode.find('ul.jstree-container-ul > li.jstree-node').each(function( index ) {
      if (index >= facets_before_more) {
       $(this).addClass('narrowGroupHidden-'+treeNode.data('facet') + ' hidden');
     }  
  });
});
...
function initFacetTree(treeNode, inSidebar, facets_before_more, buttonsHtml)
{
  ...
  $.getJSON(VuFind.path + '/AJAX/JSON?' + query,
    ...
    function getFacetData(response/*, textStatus*/) {
      buildFacetTree(treeNode, response.data.facets, inSidebar, facets_before_more);
    }
  );
}
